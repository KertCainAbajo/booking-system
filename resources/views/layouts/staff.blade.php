    ...
    <div class="min-h-screen" x-data="{ sidebarOpen: false }">
        ...
        <aside class="fixed inset-y-0 left-0 z-50 w-64 min-h-full bg-gradient-to-b from-garage-charcoal to-garage-darkgreen border-r border-garage-neon/20 shadow-garage transform transition-transform duration-300 ease-in-out lg:translate-x-0 flex flex-col"
            ...
            <div class="relative flex items-center justify-center h-16 px-4 border-b border-garage-neon/20 flex-shrink-0">
                <a href="{{ route('staff.dashboard') }}" class="flex items-center space-x-2 text-xl font-bold text-white hover:text-garage-neon transition-colors service-tag">
            ...
            <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
                <a href="{{ route('staff.dashboard') }}" 
                   class="flex items-center px-4 py-3 text-white hover:text-garage-neon hover:bg-garage-forest rounded-lg font-semibold transition-colors service-tag {{ request()->routeIs('staff.dashboard') ? 'bg-garage-forest text-garage-neon' : '' }}">
                ...
                <a href="{{ route('staff.calendar') }}" 
                   class="flex items-center px-4 py-3 text-white hover:text-garage-neon hover:bg-garage-forest rounded-lg font-semibold transition-colors service-tag {{ request()->routeIs('staff.calendar') ? 'bg-garage-forest text-garage-neon' : '' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('staff.bookings') }}" 
                   class="flex items-center px-4 py-3 text-white hover:text-garage-neon hover:bg-garage-forest rounded-lg font-semibold transition-colors service-tag {{ request()->routeIs('staff.bookings') ? 'bg-garage-forest text-garage-neon' : '' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('staff.history') }}" 
                   class="flex items-center px-4 py-3 text-white hover:text-garage-neon hover:bg-garage-forest rounded-lg font-semibold transition-colors service-tag {{ request()->routeIs('staff.history') ? 'bg-garage-forest text-garage-neon' : '' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open" @click.away="open = false"
                        ...
                        <div class="ml-3 text-left overflow-hidden">
                            ...
                            <p class="text-xs text-white/70 truncate">Staff</p>
                        </div>
                    ...
                    <div x-show="open" 
                         ...
                         style="display: none;">
                        <a href="{{ route('staff.profile') }}" class="block px-4 py-2 text-sm text-white hover:bg-garage-forest hover:text-garage-neon transition-colors">
