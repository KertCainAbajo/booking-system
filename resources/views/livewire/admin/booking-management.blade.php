    ...
    <div class="bg-gradient-to-br from-garage-charcoal to-garage-darkgreen rounded-lg shadow-garage p-8 border-l-4 border-garage-neon relative overflow-hidden mb-6">
        ...
        <div class="relative z-10">
            <div class="flex items-center space-x-4 mb-3">
                ...
                <h1 class="text-4xl font-bold text-garage-offwhite service-tag tracking-wider">BOOKING MANAGEMENT</h1>
            </div>
            <p class="text-garage-steel text-lg ml-16">View and manage all system bookings with received date and time</p>
        </div>
                ...
                <tbody class="bg-garage-charcoal/30 divide-y divide-garage-neon/10">
                    @forelse($bookings as $booking)
                        <tr class="hover:bg-garage-forest/30 transition-colors">
                            <td class="px-4 py-4 whitespace-nowrap font-bold text-garage-neon font-mono">
                                #{{ $booking->id }}
                            </td>
                            <td class="px-4 py-4">
                                <div class="text-sm">
                                    <div class="font-medium text-garage-offwhite">{{ $booking->customer->getDisplayName() }}</div>
                                    <div class="text-garage-steel">{{ $booking->customer->user->email ?? $booking->customer->email }}</div>
                                    <div class="text-garage-steel">{{ $booking->customer->user->phone ?? $booking->customer->phone }}</div>
                                </div>
                            </td>
                            <td class="px-4 py-4">
                                <div class="text-sm">
                            ...
                            <td class="px-4 py-4 whitespace-nowrap text-sm text-garage-offwhite">
                            ...
                            <td class="px-4 py-4 whitespace-nowrap">
                                ...
                                @endphp
                                <span class="px-3 py-1 text-xs rounded-full border {{ $statusColors[$booking->status] ?? 'bg-garage-steel/20 text-garage-steel border-garage-steel/30' }} font-semibold">
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="px-4 py-8 text-center text-garage-steel">
                                No bookings found.
                            </td>
                        </tr>
                    @endforelse
